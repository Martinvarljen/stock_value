    ...
    *,
    ...
    for p in state.positions:
        ...
        if p.side == "long":
            ...
            agg.by_sector_long[sector] = agg.by_sector_long.get(sector, 0.0) + p.notional
            agg.beta_weighted_long += beta * p.notional
        else:
            agg.short_notional += p.notional
            agg.by_sector_short[sector] = agg.by_sector_short.get(sector, 0.0) + p.notional
            agg.beta_weighted_short += beta * p.notional
...
def apply_pre_trade_limits(
    ...
    *,
    ...
    for d in decisions:
        ...
        notional = max(0.0, eff_frac * state.nav)
        if notional <= 0 or state.cash < notional * 0.999:
        ...
        new_long = agg.long_notional + (notional if side_long else 0.0)
        new_short = agg.short_notional + (0.0 if side_long else notional)
        new_long_sector = dict(agg.by_sector_long)
        new_short_sector = dict(agg.by_sector_short)
        if side_long:
            new_long_sector[sector] = new_long_sector.get(sector, 0.0) + notional
        else:
            new_short_sector[sector] = new_short_sector.get(sector, 0.0) + notional
        new_beta_long = agg.beta_weighted_long + (beta * notional if side_long else 0.0)
        new_beta_short = agg.beta_weighted_short + (0.0 if side_long else beta * notional)
        new_varsq = agg.var_var_squared + (notional * vol) ** 2
