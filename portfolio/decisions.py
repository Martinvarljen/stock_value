...
from portfolio.trailing_stop import stop_exit_label
...
def _tp_hit(pos: Position, price: float, cfg: dict[str, Any]) -> bool:
    if not take_profit_enabled(cfg):
        return False
    if pos.side == "long":
    ...
    *,
    ...
    if position:
        ...
        if _stop_hit(position, price_f, cfg):
            base.action = Action.EXIT
            base.reason = stop_exit_label(position)
            return base
        if _tp_hit(position, price_f, cfg):
            base.action = Action.EXIT
        ...
        if position.side == "long":
            if should_exit_long_on_regime(regime, cfg):
                base.action = Action.EXIT
                base.reason = "Regime not bull — exit long"
                return base
            exit_p = long_score_exit_threshold(regime, cfg)
            min_hold = min_hold_before_score_exit(cfg, "long")
            if (
                exit_p is not None
                and not score_exit_blocked_by_hold(held, "long", cfg)
                and p_up is not None
                and float(p_up) < exit_p
            ):
                base.action = Action.EXIT
                base.reason = f"P(up) 20d {float(p_up):.0%} < exit {exit_p:.0%}"
                return base
        else:
            if should_cover_short_on_regime(regime, cfg):
                base.action = Action.EXIT
                base.reason = "Regime not bear — cover short"
                return base
            exit_p = short_score_exit_threshold(position, cfg)
            if (
                not score_exit_blocked_by_hold(held, "short", cfg)
                and p_up is not None
                and float(p_up) > exit_p
            ):
                base.action = Action.EXIT
    ...
    if quintile is None:
        if float(p_up) >= min_long + abs_long_buffer and long_entry_allowed(regime, cfg, scale):
    ...
    q5_floor = float(cfg.get("long_p_up_quintile_5_floor", min_long))
    if long_entry_allowed(regime, cfg, scale):
        if quintile == 5 and float(p_up) >= q5_floor:
            base.action = Action.ENTER_LONG
            base.reason = f"Q5 P(up)20d={float(p_up):.0%} (top quintile)"
            return base
        if quintile is not None and quintile >= q_long and float(p_up) >= min_long:
            base.action = Action.ENTER_LONG
